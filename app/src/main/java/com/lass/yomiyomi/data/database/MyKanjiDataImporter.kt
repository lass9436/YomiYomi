...
object MyKanjiDataImporter {

    /**
     * raw 리소스 폴더에서 my_kanji_list 데이터를 읽어와 MyKanji 리스트로 변환
     */
    fun importMyKanjiFromCsv(context: Context): List<MyKanji> {
        val myKanjiList = mutableListOf<MyKanji>()

        try {
            // raw 폴더의 리소스 파일 열기
            val inputStream = context.resources.openRawResource(R.raw.my_kanji_list)
