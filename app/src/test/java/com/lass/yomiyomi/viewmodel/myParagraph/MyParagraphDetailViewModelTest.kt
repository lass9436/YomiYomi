...
class MyParagraphDetailViewModelTest {
    ...
    fun `초기 상태 확인`() = runTest {
        ...
        // Then
        assertNull(viewModel.paragraph.value)
        assertTrue(viewModel.sentences.value.isEmpty())
        assertTrue(viewModel.availableCategories.value.isEmpty())
        assertEquals(Level.values().toList(), viewModel.availableLevels.value)
    ...
    fun `loadParagraphDetail - 문단 상세 정보 로드 성공`() = runTest {
        ...
        // Then
        assertEquals(sampleParagraph, viewModel.paragraph.value)
        assertEquals(sampleSentences.sortedBy { it.orderInParagraph }, viewModel.sentences.value)
        assertEquals(sampleCategories, viewModel.availableCategories.value)
        assertFalse(viewModel.isLoading.value)
    ...
    fun `insertSentence - 새 문장 추가 성공`() = runTest {
        ...
        // Then
        assertEquals(updatedSentences.sortedBy { it.orderInParagraph }, viewModel.sentences.value)
    ...
    fun `insertSentence - 첫 번째 문장 추가 (기존 문장이 없는 경우)`() = runTest {
        ...
        // Then
        assertEquals(resultSentences, viewModel.sentences.value)
    ...
    fun `deleteSentence - 문장 삭제 성공`() = runTest {
        ...
        // Then
        assertEquals(remainingSentences.sortedBy { it.orderInParagraph }, viewModel.sentences.value)
    ...
    fun `Repository 에러 처리 - insertSentence`() = runTest {
        ...
        assertEquals(originalSentences, viewModel.sentences.value)
